...
`src/node.rs`, `src/walk.rs`, and `python/rustytree/{__init__.py,
backend.py}` exist. The remaining modules in the layout below
(`src/icechunk_store.rs`, `src/glob.rs`, `src/array.rs`,
